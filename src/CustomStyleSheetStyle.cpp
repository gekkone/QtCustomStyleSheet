#include "CustomStyleSheetStyle.h"
 ...
 */
static const CustomStyleSheetStyle *globalStyleSheetStyle = 0;
class CustomStyleSheetStyleRecursionGuard
{
public:
    CustomStyleSheetStyleRecursionGuard(const CustomStyleSheetStyle *that)
        :  guarded(globalStyleSheetStyle == 0)
    {
        if (guarded) globalStyleSheetStyle = that;
    }
    ~CustomStyleSheetStyleRecursionGuard() { if (guarded) globalStyleSheetStyle = 0; }
    bool guarded;
};
#define RECURSION_GUARD(RETURN) \
    if (globalStyleSheetStyle != 0 && globalStyleSheetStyle != this) { RETURN; } \
    CustomStyleSheetStyleRecursionGuard recursion_guard(this);
...
QHash<QStyle::SubControl, QRect> CustomStyleSheetStyle::titleBarLayout(const QWidget *w, const QStyleOptionTitleBar *tb) const
{
    ...
    : features(0), hasFont(false), pal(0), b(0), bg(0), bd(0), ou(0), geo(0), p(0), img(0), clipset(0)
{
    ...
    if (hasBorder()) {
        if (widget) {
            CustomStyleSheetStyle *style = const_cast<CustomStyleSheetStyle *>(globalStyleSheetStyle);
            if (!style)
                style = qobject_cast<CustomStyleSheetStyle *>(widget->style());
            if (style)
...
#warning FIX_PROPERTY_VALUE_CACHE
        ...
        if (!value.isValid()) {
            if (name == QLatin1String("class")) {
                ...
                CustomStyleSheetStyle *proxy = qobject_cast<CustomStyleSheetStyle *>(WIDGET(node)->style());
                if (proxy) {
...
#warning FIXME
        //FIXME наверное, в общем посмотреть можно ли так
...
int CustomStyleSheetStyle::nativeFrameWidth(const QWidget *w)
{
...
QRenderRule CustomStyleSheetStyle::renderRule(const QWidget *w, int element, quint64 state) const
{
...
QRenderRule CustomStyleSheetStyle::renderRule(const QWidget *w, const QStyleOption *opt, int pseudoElement) const
{
...
bool CustomStyleSheetStyle::hasStyleRule(const QWidget *w, int part) const
{
...
QSize CustomStyleSheetStyle::defaultSize(const QWidget *w, QSize sz, const QRect& rect, int pe) const
{
    QStyle *base = baseStyle();
...
QRect CustomStyleSheetStyle::positionRect(const QWidget *w, const QRenderRule &rule2, int pe,
...
QRect CustomStyleSheetStyle::positionRect(const QWidget *w, const QRenderRule& rule1, const QRenderRule& rule2, int pe,
...
#warning oops
    // тут раньше было закоментировано условие
...
void CustomStyleSheetStyle::setGeometry(QWidget *w)
{
...
void CustomStyleSheetStyle::setPalette(QWidget *w)
{
    struct RuleRoleMap {
        int state;
...
void CustomStyleSheetStyle::unsetPalette(QWidget *w)
{
...
int CustomStyleSheetStyle::numinstances = 0;

CustomStyleSheetStyle::CustomStyleSheetStyle(QStyle *baseStyle) : QWindowsStyle(),
...
CustomStyleSheetStyle::~CustomStyleSheetStyle()
{
    --numinstances;
    if (numinstances == 0) {
...
void CustomStyleSheetStyle::setRecursiveStyle(QWidget *_widget)
{
    if (_widget == NULL) {
        return;
    }
...
QStyle *CustomStyleSheetStyle::baseStyle() const
{
    if (base)
        return base;
#warning REMOVE BASE AND PROXY STYLE
    //    if (CustomStyleSheetStyle *me = qobject_cast<CustomStyleSheetStyle *>(QApplication::style()))
    //        return me->base;
 ...
 */
bool CustomStyleSheetStyle::initWidget(const QWidget *w) const
{
    //TODO what of?
    if (!w)
        return false;
...
void CustomStyleSheetStyle::polish(QWidget *w)
{
    if (baseStyle() != NULL) {
        baseStyle()->polish(w);
    }
...
void CustomStyleSheetStyle::polish(QApplication *app)
{
    if (baseStyle() != NULL) {
...
void CustomStyleSheetStyle::polish(QPalette &pal)
{
    if (baseStyle() != NULL) {
...
void CustomStyleSheetStyle::repolish(QWidget *w)
{
...
void CustomStyleSheetStyle::repolish(QApplication *app)
{
    Q_UNUSED(app);
...
void CustomStyleSheetStyle::unpolish(QWidget *w)
{
...
void CustomStyleSheetStyle::unpolish(QApplication *app)
{
    if (baseStyle() != NULL) {
...
void CustomStyleSheetStyle::drawComplexControl(ComplexControl cc, const QStyleOptionComplex *opt, QPainter *p,
...
void CustomStyleSheetStyle::drawControl(ControlElement ce, const QStyleOption *opt, QPainter *p,
                                        const QWidget *w) const
{
    QRenderRule rule = renderRule(w, opt);
...
#warning DELETE IN THE FUTURE
        // Старая реализация, удалить в стабильной версии
        ...
        if (subRule.hasDrawable()) {
            if (const QStyleOptionProgressBarV2 *pb = qstyleoption_cast<const QStyleOptionProgressBarV2 *>(opt)) {
                ...
                if (pb->minimum == 0 && pb->maximum == 0) {
                    ...
                    int offset = (/*d->animateStep*/1*8%rect.width());
...
void CustomStyleSheetStyle::drawItemPixmap(QPainter *p, const QRect &rect, int alignment, const
...
void CustomStyleSheetStyle::drawItemText(QPainter *painter, const QRect& rect, int alignment, const QPalette &pal,
...
void CustomStyleSheetStyle::drawPrimitive(PrimitiveElement pe, const QStyleOption *opt, QPainter *p,
                                          const QWidget *w) const
{
#warning REMOVE RECURSION_GUARD
    ...
    case PE_FrameStatusBar: {
        QRenderRule subRule = renderRule(w ? w->parentWidget() : NULL, opt, PseudoElement_Item);
        if (subRule.hasDrawable()) {
            subRule.drawRule(p, opt->rect);
            return;
        }
...
#warning FIXME
    ...
    case PE_PanelScrollAreaCorner:
        ...
        return;
#warning TODO
        //TODO Нормально отрисовывать, подсмотрев в каком нибудь из стилей
        // именно из за этой штуки криво рисуются фреймы вокруг скролируемых областей
        break;
...
QPixmap CustomStyleSheetStyle::generatedIconPixmap(QIcon::Mode iconMode, const QPixmap& pixmap,
...
QStyle::SubControl CustomStyleSheetStyle::hitTestComplexControl(ComplexControl cc, const QStyleOptionComplex *opt,
...
QRect CustomStyleSheetStyle::itemPixmapRect(const QRect &rect, int alignment, const QPixmap &pixmap) const
{
    if (baseStyle() == NULL) {
        return QRect();
    }
...
QRect CustomStyleSheetStyle::itemTextRect(const QFontMetrics &metrics, const QRect& rect, int alignment,
...
int CustomStyleSheetStyle::pixelMetric(PixelMetric m, const QStyleOption *opt, const QWidget *w) const
{
#warning REMOVE RECURSION_GUARD
...
QSize CustomStyleSheetStyle::sizeFromContents(ContentsType ct, const QStyleOption *opt,
...
QIcon CustomStyleSheetStyle::standardIconImplementation(StandardPixmap standardIcon, const QStyleOption *opt,
                                                        const QWidget *w) const
{
...
QPalette CustomStyleSheetStyle::standardPalette() const
{
    if (baseStyle() != NULL) {
...
QPixmap CustomStyleSheetStyle::standardPixmap(StandardPixmap standardPixmap, const QStyleOption *opt,
                                              const QWidget *w) const
{
...
int CustomStyleSheetStyle::layoutSpacing(QSizePolicy::ControlType control1, QSizePolicy::ControlType control2,
...
int CustomStyleSheetStyle::layoutSpacingImplementation(QSizePolicy::ControlType  control1 ,
...
int CustomStyleSheetStyle::styleHint(StyleHint sh, const QStyleOption *opt, const QWidget *w,
...
QRect CustomStyleSheetStyle::subControlRect(ComplexControl cc, const QStyleOptionComplex *opt, SubControl sc,
                                            const QWidget *w) const
{
...
QRect CustomStyleSheetStyle::subElementRect(SubElement se, const QStyleOption *opt, const QWidget *w) const
{
...
bool CustomStyleSheetStyle::event(QEvent *e)
{
    if (baseStyle() != NULL) {
...
void CustomStyleSheetStyle::saveWidgetFont(QWidget* w, const QFont& font) const
{
    w->setProperty("_q_styleSheetWidgetFont", font);
}

void CustomStyleSheetStyle::clearWidgetFont(QWidget* w) const
{
...
bool CustomStyleSheetStyle::styleSheetPalette(const QWidget* w, const QStyleOption* opt, QPalette* pal)
{
    if (!w || !opt || !pal)
        return false;
...
bool CustomStyleSheetStyle::isNaturalChild(const QWidget *w)
{
